...
namespace Framework.ViewModel
{
    public class MenuCommands : BaseVM
    {
        ...
        List<string> parametersB = new List<string>
        {
            "Brightness value: ",         
        };

        
        private void Brightness(object parameter)
        {

            DialogBox window = new DialogBox(_mainVM, parametersB);
            ...
            if (values!=null)
            {
                 double brightnessValue = values.Count > 0 ? values[0] : 0;
                if (ColorInitialImage != null)
                {
                    ColorProcessedImage = PointwiseOperations.AdjustBrightness(ColorInitialImage, brightnessValue);
                    ProcessedImage = Convert(ColorProcessedImage);
                }
                else if (GrayInitialImage != null)
                {
                    GrayProcessedImage =  PointwiseOperations.AdjustBrightness(GrayInitialImage, brightnessValue);
                    ProcessedImage = Convert(GrayProcessedImage);
                }      
            }     
        }
        ...
        List<string> parametersC = new List<string>
        {
            "Contrast value: ",
        };
        private void Contrast(object parameter)
        {
            DialogBox window = new DialogBox(_mainVM, parametersC);
            var result = window.ShowDialog();
            ...
            if (values!=null)
            {
                double contrastValue = values.Count > 0 ? values[0] : 0;
                if (ColorInitialImage != null)
                {
                    ColorProcessedImage = PointwiseOperations.AdjustBrightnessAndContrast(ColorInitialImage, contrastValue);
                    ProcessedImage = Convert(ColorProcessedImage);
                }
                else if (GrayInitialImage != null)
                {
                    GrayProcessedImage =  PointwiseOperations.AdjustBrightnessAndContrast(GrayInitialImage, contrastValue);
                    ProcessedImage = Convert(GrayProcessedImage);
                }
            }
        }
    ...
    #region Thresholding
        ...
        #region Intermeans
        private ICommand _intermeansBinarizationCommand;
        public ICommand IntermeansBinarizationCommand
        {
            get
            {
                if (_intermeansBinarizationCommand == null)
                    _intermeansBinarizationCommand = new RelayCommand(IntermeansThresholdingImage);
                return _intermeansBinarizationCommand;
            }
        }


        private void IntermeansThresholdingImage(object parameter)
        {
            if (InitialImage == null)
            {
                MessageBox.Show("Please add an image!");
            ...
            if (GrayInitialImage != null)
            {
                byte threshold = Thresholding.CalculateIntermeansThreshold(GrayInitialImage);
            ...
            else if (ColorInitialImage != null)
            {
                // Conversie BGR -> Grayscale
                GrayProcessedImage = Tools.Convert(ColorInitialImage);
                byte threshold = Thresholding.CalculateIntermeansThreshold(GrayProcessedImage);
                GrayProcessedImage = Tools.Thresholding(GrayProcessedImage, threshold);
                ProcessedImage = Convert(GrayProcessedImage);
            }
        ...
        #region Filters
        #region Kuwahara
        private ICommand _kuwaharafilterCommand;
        public ICommand KuwaharaFilterCommand
        {
            get
            {
                if (_kuwaharafilterCommand == null)
                    _kuwaharafilterCommand = new RelayCommand(KuwaharaFilterImage);
                return _kuwaharafilterCommand;
            }
        }


        private void KuwaharaFilterImage(object parameter)
        {
            try
            {
                if (InitialImage == null)
                ...
                if (GrayInitialImage != null)
                {
                    GrayProcessedImage = Filters.KuwaharaFilter(GrayInitialImage, windowSize);
                    ProcessedImage = Convert(GrayProcessedImage);
                }
                else
                {
        ...
        #region XOR
        private ICommand _XORCommand;
        public ICommand MorphologicalOperationsCommand
        {
            get
            {
                if (_XORCommand == null)
                    _XORCommand = new RelayCommand(MorphologicalOperationsImage);
                return _XORCommand;
            }
        }

        private byte[,] ConvertImageToArray(Image<Gray, byte> image)
        {
            int width = image.Width;
            int height = image.Height;

            byte[,] resultArray = new byte[width, height];

            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    resultArray[x, y] = image.Data[y, x, 0];
                }
            }
        ...
        private Image<Gray, byte> ConvertArrayToImage(byte[,] array, int width, int height)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    resultImage.Data[y, x, 0] = array[x, y];
                }
            }
        ...
        private void MorphologicalOperationsImage(object parameter)
        {
            try
            {
                if (GrayInitialImage == null)
                ...
                // Conversie in matrice de octeti
                byte[,] grayImageArray = ConvertImageToArray(GrayInitialImage);
      
                int erosionIterations = 4; 
                byte[,] processedImageArray = MorphologicalOperations.PerformXOR(grayImageArray, GrayInitialImage.Width, GrayInitialImage.Height, erosionIterations);

                // Coversie inapoi la Image<Gray, byte>
                GrayProcessedImage = ConvertArrayToImage(processedImageArray, GrayInitialImage.Width, GrayInitialImage.Height);
        ...
        private ICommand _twirlCommand;
        public ICommand TwirlCommand
        {
            get
            {
                if (_twirlCommand == null)
                    _twirlCommand = new RelayCommand(Twirl);
                return _twirlCommand;
            }
        }

        List<string> parametersTwirl = new List<string>
        {
            "Twirl value: ",
        };


        private void Twirl(object parameter)
        {

            DialogBox window = new DialogBox(_mainVM, parametersTwirl);
            var result = window.ShowDialog();
            ...
            if (ColorInitialImage != null)
            {
                ...
                {
                    double twirlValue = values[0];
                    Image<Bgr, byte> twirledImage = GeometricTransformations.ApplyTwirl(ColorInitialImage, twirlValue);       
                    ProcessedImage = Convert(twirledImage);
            ...
            else if (GrayInitialImage != null)
            {
                if (values != null && values.Count > 0)
                {
                    ...
                    Image<Gray, byte> twirledImageg = GeometricTransformations.ApplyTwirl(GrayInitialImage, twirlValue);

                    ProcessedImage = Convert(twirledImageg);
